...
canvasdisplay.prototype.drawRollingBackground = function(name,X,y){
	let background = this.spriteList[name];
	if(!background.scale){
		this.cx.save();
		this.cx.scale(1,1);
	}
	...
	let wB=background.w;
	let hB=background.h;
	let wC=Math.ceil(this.canvas.width/_SCALE);
	let dX=0;
	let leftX=Math.floor(mod(X,wB)+wB/2-wC/2);
	let rightX=leftX+wC;
	let x=0;
	if(leftX<0){
		dX=Math.abs(leftX);
		x=dX;
		this.cx.drawImage(img,wB-dX,0,dX,hB,0,y,dX,hB);
	}else if(rightX>wB){
		dX=rightX-wB;
		x=0;
		this.cx.drawImage(img,0,0,dX,hB,wC-dX,y,dX,hB);
		rightX=wB;	
	}
	this.cx.drawImage(img,leftX,0,rightX-leftX,hB,x,y,rightX-leftX,hB);
	if(!background.scale)
		this.cx.restore();
};
...
canvasdisplay.prototype.drawActors = function(actors){
	actors.forEach(function(actor){
		if (actor.show){
        	...
        	this.cx.drawImage(actorSprite.img,actor.costume*actorSprite.w+((actor.costume==0)?0:0),0,actorSprite.w,actorSprite.h,x,y,actorSprite.w,actorSprite.h);
