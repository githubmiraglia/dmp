// JavaScript Document
function clyde(sprite,costume,x,y,vx,vy,rotation,flip,show,am){
...
clyde.prototype.escape=function(clock){
	let l=this.gameData.level;
	let p=this.gameData.pellets;
	if(!(l>2||(l==1&&p<184)||(l==2&&p<194))){
...
clyde.prototype.act=function(clock,gameData,gameFlags){
				...
				if(g=="Play"){
					if(this.turnaround){
						...
						if(this.status=="Scared"){
							...
							let distX=this.gx-this.gameData.pmX;
							let distY=this.gy-this.gameData.pmY;
							let distToPM=Math.sqrt(Math.pow(distX,2)+Math.pow(distY,2));
							if(!(Math.abs(this.gx)==12&&(this.gy==28||this.gy==-68)&&(mod(this.direction,2)==1))){
								if(this.gameData.mode=="Scatter"||distToPM<64){
							   		this.turn(-108,-140); 
								}
								else{
