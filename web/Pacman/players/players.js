...
players.prototype.choseNumberPlayers=function(){
	let fadeScreen="pacmanfront";
	let initialScreen = "pacmanTopScores";
	let costume = 0;
	this.dh.hold=false;
	var interval = setInterval(function(){
		if(!this.cd.crossFaded){
			...
			if(!this.dh.chosing||this.once[5]){
				...
				if(this.am.loaded){
					...
					this.dh.hold=true;
					this.cd.drawString("...LOADING SOUNDS...",24,-50,"center","red","black");
				}
			}
		}
		this.clock++;
...
players.prototype.start=function(){
	...
	this.soundStack.push({command:"PLAY",music:"coin"});
	this.soundStack = this.am.playStack(this.soundStack);
	var interval = setInterval(function(){
		if(this.am.checkEnded("coin")){
			clearInterval(interval);
			this.cl.ind=0;
			this.dh.removeChose();
			if(this.players==2)
		...
		elapsed++;
		if(elapsed>10){
			this.cd.drawString("...BROWSER  NOT  ALLOWING  SOUNDS...",24,-50,"center","red","black");
		}
		if(elapsed>35){
			_SOUNDS=false;
		}
...
players.prototype.writeTopScores=function(){
	for(var i=0;i<_TOPSCORES.length;i++){
		...
		this.cd.drawNumber(player.score,6,53-i*20,"left","#0c70b0","black",57);
		this.cd.drawString(player.name,24,53-i*20,"left","#0c70b0","black",null,0,125);
	}
}
...
players.prototype.finishAsk=function(){
	...
	if(this.players==1){
		...
		this.cd.spriteList["stage"].img.addEventListener("load",_F);
	}else{
	...
	var interval=setInterval(function(){
		if(!this.cd.crossFaded&&_FREEZEIMAGELOADED){
			if(!this.once[9]){
				...
				this.cd.crossfade("stage","pacmanTopScores",0.8,0.8)
				this.once[9]=true;
			}
		}else if(this.cd.crossFaded){
			this.cd.drawBackground("pacmanTopScores",0);
			this.writeTopScores();
			this.cl.runChaseLight(this.clock);
...
players.prototype.choseJS=function(){
	...
	drawDevices=function(_this,player1js,player2js){
		...
		if(player1js!=-1)
			_this.cd.drawSingleActor("devices",player1js,-100,110);
		if(player2js!=-1)
			_this.cd.drawSingleActor("devices",player2js,100,110);
		for(let key in center){
			if(center.hasOwnProperty(key))
				if(center[key])
					_this.cd.drawSingleActor("devices",key,0,95-key*75);
		}
	}
