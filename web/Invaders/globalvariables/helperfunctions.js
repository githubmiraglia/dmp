...
function checkColor(ctx,x,y,w,h,bol){
	let x1 = Math.ceil((x*_GAMEWIDTH/_K[2]+_GAMEWIDTH/2));
	let y1 = Math.ceil((-y*(_GAMEHEIGHT-80)/_K[3]+_GAMEHEIGHT/2)-100)+2;
	//ctx.fillStyle="green";
	//ctx.fillRect(x1,y1,w,h);
	let p=ctx.getImageData(Math.ceil(x1*_SCALE),Math.ceil(y1*_SCALE),w,h).data; 
	for(let i=0;i<(p.length/4);i+=4){
		if(p[i]>=225&&p[i]<=255&&p[i+1]<=30&&p[i+2]<=30){
			return("shieldhit");
		}
	}
	return(bol);
