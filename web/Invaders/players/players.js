...
players.prototype.choseNumberPlayers=function(){
	let fadeScreen="invadersFront";
	let initialScreen = "invadersTopScores";
	let costume = 0;
	this.dh.hold=false;
	var interval = setInterval(function(){
...
players.prototype.start=function(){
	...
	this.soundStack.push({command:"PLAY",music:"pop"});
	this.soundStack = this.am.playStack(this.soundStack);
	var interval = setInterval(function(){
		if(this.am.checkEnded("pop")){
			clearInterval(interval);
			this.cl.ind=0;
			this.dh.removeChose();
			if(this.players==2)
	...
	var interval=setInterval(function(){
		if(!this.cd.crossFaded&&_FREEZEIMAGELOADED){
			if(!this.once[9]){
				...
				this.cd.crossfade("stage","invadersTopScores",0.8,0.8);
				this.once[9]=true;
			}
		}else if(this.cd.crossFaded){
			this.cd.drawBackground("invadersTopScores",0);
			this.writeTopScores();
			this.cl.runChaseLight(this.clock);
...
players.prototype.readTopScores=function(){
	...
	if(navigator.onLine){
		new Promise(function(resolve,reject){
			this.sv.readJSON("topscores","gameId",4).then(function(result){
